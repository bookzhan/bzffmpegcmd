...
static void flush_encoders(void) {
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (enc->codec_type == AVMEDIA_TYPE_AUDIO && enc->frame_size <= 1)
            continue;
#if FF_API_LAVF_FMT_RAWPICTURE
        if (enc->codec_type == AVMEDIA_TYPE_VIDEO && (of->ctx->oformat->flags & AVFMT_RAWPICTURE) &&
            enc->codec->id == AV_CODEC_ID_RAWVIDEO)
            continue;
#endif
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt) {
    ...
    if (ost->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
        ...
        int ret = avpicture_fill(&pict, opkt.data, ost->st->codecpar->format,
                                 ost->st->codecpar->width, ost->st->codecpar->height);
        if (ret < 0) {
    ...
    if (ost->encoding_needed) {
        ...
        if (ost->filter && ost->filter->filter->inputs[0]->hw_frames_ctx) {
            ost->enc_ctx->hw_frames_ctx = av_buffer_ref(
                    ost->filter->filter->inputs[0]->hw_frames_ctx);
...
static int transcode(void) {
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error writing trailer of %s: %s", os->filename,
                   av_err2str(ret));
            if (exit_on_error)
                return exit_program(-1);
        }
