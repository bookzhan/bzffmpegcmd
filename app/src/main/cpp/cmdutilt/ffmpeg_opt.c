...
new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index) {
    ...
    while (bsfs && *bsfs) {
        ...
        if (bsf_options_str && filter->priv_class) {
            const AVOption *opt = av_opt_next(
                    ost->bsf_ctx[ost->nb_bitstream_filters - 1]->priv_data, NULL);
            const char *shorthand[2] = {NULL};

            if (opt)
                shorthand[0] = opt->name;

            ret = av_opt_set_from_string(ost->bsf_ctx[ost->nb_bitstream_filters - 1]->priv_data,
    ...
    if (ost->nb_bitstream_filters) {
        ost->bsf_extradata_updated = av_mallocz_array(ost->nb_bitstream_filters,
        ...
        if (!o->video_disable &&
            ...
            for (i = 0; i < nb_input_streams; i++) {
                ...
                new_area = ist->st->codecpar->width * ist->st->codecpar->height +
                           100000000 * !!ist->st->codec_info_nb_frames;
...
int ffmpeg_parse_options(int argc, char **argv) {
    ...
    int i = 0;
    for (i; i < argc; i++) {
        av_log(NULL, AV_LOG_DEBUG, "argv---argv[%d]=%s", i, argv[i]);
    }
